                             ...
                             VkSemaphore* semaphore)
{
    ...
    VkResult result =
        vkCreateSemaphore(device->device, &create_info, NULL, semaphore);
    if (result != VK_SUCCESS)
    {
        fprintf(
            stderr,
            "[ERROR]: Failed to create a Vulkan semaphore. Vulkan error %d.\n",
            result);
        return false;
    }
...
bool create_vulkan_fence(const struct device* device, VkFence* fence)
{
    ...
    {
        fprintf(stderr,
                "[ERROR]: Failed to create a Vulkan fence. Vulkan error %d.\n",
                result);
        return false;
    }
