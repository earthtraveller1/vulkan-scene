...
auto choose_physical_device(VkInstance p_instance,
    ...
    -> result_t<physical_device, kirho::empty> {
...
auto create_debug_messenger(VkInstance p_instance) noexcept
    -> result_t<VkDebugUtilsMessengerEXT, VkResult> {
...
auto main() noexcept -> int {
  ...
  const auto debug_messenger = enable_validation
                                   ? create_debug_messenger(instance).unwrap()
                                   : VK_NULL_HANDLE;
  defer(debug_messenger,
        enable_validation ? destroy_debug_messenger(instance, debug_messenger)
                          : (void)0);
  ...
  const auto [physical_device, graphics_queue_family, present_queue_family] =
      choose_physical_device(instance, surface).unwrap();

  const auto logical_device =
      create_logical_device(physical_device, graphics_queue_family,
