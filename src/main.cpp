...
auto create_semaphore(VkDevice p_device) noexcept
  ...
  if (result != VK_SUCCESS) {
    vulkan_scene::print_error("Failed to create a semaphore. Vulkan error ",
                              result, '.');
    return result_tt::error(result);
  }
...
auto create_fence(VkDevice p_device) noexcept -> result_t<VkFence, VkResult> {
  ...
  VkFence fence;
  const auto result = vkCreateFence(p_device, &semaphore_info, nullptr, &fence);
  if (result != VK_SUCCESS) {
    vulkan_scene::print_error("Failed to create a fence. Vulkan error ", result,
                              '.');
    return result_tt::error(result);
  }
...
auto main() noexcept -> int {
  ...
  const auto vertex_shader_module =
      create_shader_module(logical_device, "shaders/basic.vert.spv").unwrap();
  ...
  const auto fragment_shader_module =
      create_shader_module(logical_device, "shaders/basic.frag.spv").unwrap();
  ...
  const auto graphics_pipeline =
      create_graphics_pipeline(logical_device, render_pass, pipeline_layout,
                               vertex_shader_module, fragment_shader_module)
          .unwrap();
  defer(graphics_pipeline,
